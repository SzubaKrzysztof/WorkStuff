...
$(document).on('click', '.meny-overlay', () => {
	...
	$('.meny-overlay').css('visibility', 'hidden');
	$('.meny-overlay').css('opacity', '0');
})
	...
	create: function( options ) {
		return (function(){
			...
			var config = {
				...
				gradient: 'rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.60) 70%)',
				coverClass: 'meny-overlay',
				mouse: options.mouse,
				zIndex: 60,
				touch: options.touch
			};
			...
			function setupPositions() {
				...
				switch( config.position ) {
					...
					case POSITION_R:
						...
						menuTransformClosed = 'rotateY( ' + menuAngle + 'deg ) translateX( 102% ) translateX( -2px ) scale( 1.01 )';
					...
					default:
						// Primary transform:
						menuTransformOrigin = '100% 50%';
						menuTransformClosed = 'translateX( -105% ) translateX( '+ config.overlap +'px ) scale( 1.01 ) rotateY( ' + -menuAngle + 'deg )';
			...
			function setupWrapper() {
				...
				Meny.removeClass( dom.wrapper, 'meny-' + config.position );
			 ...
			 */
			function open() {
				if( !isOpen && !$('.menu-categories').hasClass('active')) {
					isOpen = true;
